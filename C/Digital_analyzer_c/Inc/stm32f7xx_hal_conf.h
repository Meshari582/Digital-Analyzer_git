...
/* Stays on despite stm32f7xx_hal_flash.c not being in CMakeLists.txt: no
 * HAL_FLASH_* function is ever called (register access is bare-metal
 * everywhere else in this project too), but HAL_Init() itself calls the
 * __HAL_FLASH_ART_ENABLE()/__HAL_FLASH_PREFETCH_BUFFER_ENABLE() macros
 * (register-only, no .c needed) under PREFETCH_ENABLE/ART_ACCELERATOR_ENABLE
 * below - those macros are declared in stm32f7xx_hal_flash.h, which this
 * define is what gates including. Turning this off breaks the build. */
#define HAL_FLASH_MODULE_ENABLED
/* Everything below is unused so far in this project (added incrementally
   as each build-order step needs it) */
...
/**
  ...
  */
  ...
  #define HSE_VALUE    8000000U /*!< NUCLEO-F767ZI: 8 MHz from ST-LINK MCO */
#endif /* HSE_VALUE */
